...
async def run(args: argparse.Namespace) -> None:
    ...
    elif args.startup_source:
        if "ycombinator.com/companies" in args.startup_source:
            records["startups"] = await YCStartupCrawler(args.startup_source).crawl(args.startup_limit)
        else:
            records["startups"] = await PublicDirectoryCrawler("configured startup directory", args.startup_source, "startup").crawl(args.startup_limit)
        if not records["startups"]:
            logging.warning("startup_source_returned_zero_records source=%s", args.startup_source)
    if args.products:
    ...
    elif args.product_source:
        if "producthunt.com" in args.product_source:
            ...
            if os.getenv("PRODUCT_HUNT_TOKEN"):
                try:
                    ...
                    source_events.append({"source": "Product Hunt API", "url": args.product_source, "status": getattr(error, "status", None), "block_type": "authentication_failed" if "authentication" in str(error).lower() else "request_failed", "error": str(error)})
                    records["products"] = await crawler.crawl_public(args.product_limit)
            else:
                records["products"] = await crawler.crawl_public(args.product_limit)
            if len(records["products"]) < args.product_limit:
                remaining = args.product_limit - len(records["products"])
                try:
                    fallback_records = await GitHubProductCrawler().crawl(remaining)
                    records["products"].extend(fallback_records)
            ...
            records["products"] = await PublicDirectoryCrawler("configured product directory", args.product_source, "product").crawl(args.product_limit)
        if not records["products"]:
            logging.warning("product_source_returned_zero_records source=%s", args.product_source)
    ...
    if args.startups or args.products or args.startup_source or args.product_source:
    ...
    if args.extract_news:
        ...
        for index, record in enumerate(records["news"]):
            extracted = await orchestrator.extract(record.content, {"summary": "string or null", "topics": "array of strings", "companies": "array of strings"})
            records["news"][index] = record.model_copy(update={"fields": extracted, "extraction_timestamp": datetime.now(timezone.utc), "extraction_provider": orchestrator.last_provider})
